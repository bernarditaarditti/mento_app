...
export default function RelationshipsLevel4GamePage() {
  const { user } = useAuth()
  // Verificar localStorage al inicio
  const [isLevelCompleted, setIsLevelCompleted] = useState(() => {
    if (typeof window !== "undefined") {
      return localStorage.getItem(`relationships_level_4_completed`) === "true"
    }
    return false
  })
  const levelNumber = 4
  ...
  // Recargar progreso cuando la página vuelve a tener foco
  useEffect(() => {
    if (typeof window === "undefined") return
    if (!user?.id_usuario) return
  ...
  useEffect(() => {
    if (typeof window === "undefined") return
    const completed = localStorage.getItem(`relationships_level_${levelNumber}_completed`) === "true"
    if (completed) {
      setIsLevelCompleted(true)
    }
  }, [levelNumber])

  const starColor = isLevelCompleted ? "#FFBC03" : "#9CA3AF"
  ...
  return (
    <div className="min-h-screen flex flex-col bg-white" style={{ height: "100vh", maxHeight: "100vh", overflow: "hidden" }}>
      ...
      <div className="flex-1 px-4 pb-3 flex flex-col gap-10 max-w-sm mx-auto w-full overflow-y-auto pt-8" style={{ minHeight: 0 }}>
        {/* Pink Box - Top */}
        <div
          className="rounded-xl flex-shrink-0 relative mt-6 mx-auto"
          style={{ backgroundColor: "rgba(255, 199, 209, 0.82)", maxWidth: "75%", padding: "20px 16px" }}
        >
          {/* Level Indicator - Centered above pink box, overlapping top edge - ensure full visibility */}
          <div className="absolute left-1/2 transform -translate-x-1/2" style={{ zIndex: 10, top: "-32px" }}>
            <div className="relative">
              ...
              <div
                ...
                style={{ backgroundColor: "#FF916D", fontFamily: "var(--font-poppins)" }}
              >
                4
              </div>
            </div>
          ...
          <div className="flex flex-col gap-2 mt-6" style={{ color: "#FF8FA3", fontFamily: "var(--font-poppins)" }}>
            ...
            <div className="text-center mt-2 mx-auto" style={{ maxWidth: "85%" }}>
              <p
        ...
        <div
          ...
          style={{ backgroundColor: "#C7EAF5", maxWidth: "75%", paddingTop: "2.5rem" }}
        >
          ...
          <div className="flex flex-col gap-2 mt-2" style={{ color: "#0096C7", fontFamily: "var(--font-poppins)" }}>
            ...
            <div className="flex flex-col gap-1.5 font-normal text-center" style={{ fontSize: "16px", lineHeight: "1.5" }}>
              <p>
                Se te presentará una situación difícil en el ámbito de pareja
              </p>
              <p className="mt-2">
                Para ese escenario, deberás seleccionar los pensamientos negativos y conectarlos con su reinterpretación positiva correspondiente
              </p>
            </div>
          </div>
            
          {/* Play Button - Inside blue box, half extending below the box */}
          <div className="flex justify-center mt-4 flex-shrink-0" style={{ marginBottom: "-32px" }}>
            <Link href="/islands/relationships/level-4/game/play">
              <button
