...
export default function relationshipsLevel3PlayPage() {
  ...
  // Opción correcta: "Capaz está abrumado o pendiente de algo importante." (índice 1)
  // Opciones incorrectas: "No le importo, me está ignorando." (índice 0), "Lo hace para molestarme." (índice 2)
  ...
  const handleOptionClick = (index: number) => {
    ...
    // Selección única: si ya está seleccionada, deseleccionarla; si no, seleccionarla
  ...
  const handleSubmit = () => {
    ...
    if (correctAnswers.includes(selectedOption)) {
      // Opción correcta - éxito
      setShowSuccessPopup(true)
      localStorage.setItem("relationships_level_3_completed", "true")
    } else {
  ...
  // Verificar si hay opción incorrecta seleccionada (después de evaluar)
  ...
  const getOptionStyle = (index: number) => {
    ...
    // Si está seleccionada pero aún no evaluada, mostrar color
  ...
  const options = [
    ...
    "Capaz está abrumado o pendiente de algo importante",
  ...
  return (
    <div className="min-h-screen flex flex-col bg-white">
      ...
      <div className="flex-1 px-4 pb-8 flex flex-col max-w-sm mx-auto w-full pt-0">
        ...
        >
          <div className="flex flex-col gap-3" style={{ color: "#F36E86", fontFamily: "var(--font-poppins)" }}>
            <p className="font-medium text-center" style={{ fontSize: "15px" }}>
              En una reunión social, un amigo mira mucho el celular. <span className="font-bold">Esto te hace sentir rechazado/a, porque pensás que ya no le interesás o que te está ignorando a propósito</span>
            </p>
          </div>
        </div>
        ...
        >
          Seleccioná la opción más constructiva
        </h2>

        {/* Options - Column */}
        <div className="flex flex-col gap-3 relative max-w-[90%] mx-auto" style={{ marginTop: "24px" }}>
            ...
            <button
              ...
              onMouseEnter={(e) => {
                ...
                if (evaluated) {
                  ...
                  if ((incorrectAnswers.includes(index) && selectedOption === index) || 
                      (correctAnswers.includes(index) && selectedOption === index)) {
