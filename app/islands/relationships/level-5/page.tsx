...
export default function RelationshipsLevel5Page() {
  ...
  useEffect(() => {
    ...
    for (let i = 1; i <= 5; i++) {
      if (localStorage.getItem(`relationships_level_${i}_completed`) === "true") {
        completed.add(i)
      }
    }
    if (completed.size > 0) {
  ...
  const handleBackgroundClick = (e: React.MouseEvent<HTMLDivElement>) => {
    // Redirigir a islands/relationships al hacer click en el fondo
      ...
      <div className="flex-1 relative px-4 pb-24 pt-2">
        ...
        {Array.from({ length: totalLevels }, (_, i) => {
          ...
          return (
            ...
            >
              ...
              <div
                className={`w-16 h-16 rounded-full flex items-center justify-center text-white font-bold text-xl shadow-lg select-none ${!isLevelUnlocked(level) ? 'level-locked-relationships' : ''}`}
                style={{
                  backgroundColor: getLevelColor(level),
                  boxShadow: level === 1 
                    ? "0 4px 12px rgba(255, 124, 109, 0.3)"
                    : "0 4px 12px rgba(254, 186, 178, 0.3)",
                  fontFamily: "var(--font-poppins)",
                  cursor: !isLevelUnlocked(level) ? "not-allowed" : "pointer",
                  WebkitTapHighlightColor: "transparent",
                  userSelect: "none",
                  pointerEvents: !isLevelUnlocked(level) ? "none" : "auto",
                }}
                onClick={(e) => {
                  if (!isLevelUnlocked(level)) {
                    e.preventDefault()
        ...
        >
          ...
          >
            ...
            {/* Content */}
            <div className="flex flex-col items-center gap-1.5" style={{ color: "#FF8FA3", fontFamily: "var(--font-poppins)" }}>
              {/* NIVEL 5 */}
              <h2
                className="font-bold uppercase"
                style={{ fontSize: "32px", fontWeight: "900", color: "#F36E86" }}
              >
                NIVEL 5
              </h2>
              ...
              <p
                className="font-bold"
                style={{ fontSize: "20px", fontWeight: "900", color: "#F36E86" }}
              >
                Reappraisal Interpersonal
              </p>
              ...
              <div className="flex justify-center mt-2" style={{ marginBottom: "-32px" }}>
                <Link href="/islands/relationships/level-5/game">
                <button
