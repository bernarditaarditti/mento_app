...
export default function HealthLevel5PlayPage() {
  ...
  // Opciones correctas (reappraisal inventivo): índices 1, 2, 4
  ...
  const handleOptionClick = (index: number) => {
    ...
    // Toggle la selección de la opción
  ...
  const handleSubmit = () => {
    // Debe haber exactamente 2 opciones seleccionadas
    if (selectedOptions.length !== 2) return
    
    // Evaluar las opciones seleccionadas
    setEvaluated(true)
    ...
    // Verificar si hay al menos 2 opciones correctas seleccionadas
    const correctSelected = selectedOptions.filter(opt => correctAnswers.includes(opt))
    
    if (correctSelected.length === 2) {
      // Exactamente 2 opciones correctas seleccionadas - éxito
      setShowSuccessPopup(true)
      localStorage.setItem("health_level_5_completed", "true")
    } else {
      // No hay suficientes opciones correctas
      setShowErrorPopup(true)
    }
  }
  ...
  const hasIncorrectOption = evaluated && selectedOptions.some(opt => incorrectAnswers.includes(opt))

  const getOptionStyle = (index: number) => {
    // Si ya se evaluó, mostrar colores según si está seleccionada
    ...
    // Si está seleccionada pero aún no evaluada, mostrar color de selección
    if (selectedOptions.includes(index) && !evaluated) {
      return { backgroundColor: "#00B46F" } // Color cuando está seleccionada
    }
    
    // Estado inicial
    return { backgroundColor: "#8CDDBE" }
  }
  ...
  return (
    <div className="min-h-screen flex flex-col bg-white">
      ...
      <div className="flex-1 px-4 pb-8 flex flex-col gap-5 max-w-sm mx-auto w-full justify-center pt-0">
        ...
        >
          <div className="flex flex-col gap-3 text-center" style={{ color: "#F36E86", fontFamily: "var(--font-poppins)" }}>
            <p className="text-sm font-medium">
              Te duele la cabeza y estás cansado
            </p>
          </div>
        </div>
        ...
        >
          Seleccioná 2 reinterpretaciones útiles entre las 6 opciones
        </h2>

        {/* Options - Grid 2x3 */}
        <div className="grid grid-cols-2 gap-2 relative max-w-[90%] mx-auto">
          {options.map((option, index) => (
            <button
              ...
              className="rounded-xl p-2.5 text-white text-center font-normal transition-colors flex items-center justify-center"
              style={{
              ...
              onMouseEnter={(e) => {
                if (!evaluated && !selectedOptions.includes(index)) {
                  e.currentTarget.style.backgroundColor = "#00B46F"
                }
                // Mostrar popup si está evaluado
                if (evaluated) {
                  // Para opciones incorrectas: siempre mostrar si están seleccionadas
                  // Para opciones correctas: solo si fue seleccionada
                  if ((incorrectAnswers.includes(index) && selectedOptions.includes(index)) || 
                      (correctAnswers.includes(index) && selectedOptions.includes(index))) {
              ...
              onMouseLeave={(e) => {
                if (!evaluated && !selectedOptions.includes(index)) {
                  e.currentTarget.style.backgroundColor = "#8CDDBE"
                }
                setHoveredOption(null)
              }}
            >
            ...
            >
              ...
              <div className="flex flex-col gap-3" style={{ fontFamily: "var(--font-poppins)" }}>
                ...
                <p
                  ...
                  style={{ fontSize: "16px" }}
                >
                  Parece difícil ver alternativas cuando estamos decepcionados. Buscar un nuevo propósito para ese tiempo puede aliviar la reacción negativa
                </p>
              </div>
            </div>
          </div>
            ...
            >
              ...
              <div className="flex flex-col gap-3" style={{ fontFamily: "var(--font-poppins)" }}>
                ...
                <p
                  ...
                  style={{ fontSize: "16px" }}
                >
                  Cambiar la meta original por una alternativa valiosa ayuda a mantener el bienestar y reduce el impacto emocional
                </p>
              </div>
            </div>
          </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white px-4 pt-4 pb-8 flex justify-center items-center max-w-md mx-auto">
        {hasIncorrectOption ? (
          ...
          <button
            ...
            style={{ backgroundColor: "#00B46F", marginBottom: "5px" }}
          >
          ...
          <div className="flex justify-center items-center gap-28 w-full">
            ...
            <div 
              ...
              onClick={() => {
                if (typeof window !== "undefined") {
                  sessionStorage.setItem("reappraisal_inventivo_return_path", pathname)
                }
                router.push("/reappraisal-inventivo")
              }}
            >
            ...
            <button
              ...
              disabled={evaluated || selectedOptions.length !== 2}
