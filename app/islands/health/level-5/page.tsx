...
export default function HealthLevel5Page() {
  ...
  useEffect(() => {
    ...
    for (let i = 1; i <= 5; i++) {
      if (localStorage.getItem(`health_level_${i}_completed`) === "true") {
        completed.add(i)
      }
    }
    if (completed.size > 0) {
    ...
    const level1Completed = localStorage.getItem("health_level_1_completed")
    const level2Completed = localStorage.getItem("health_level_2_completed")
    const level3Completed = localStorage.getItem("health_level_3_completed")
    const level4Completed = localStorage.getItem("health_level_4_completed")
  ...
  const getLevelColor = (level: number) => {
    // Si está bloqueado: color tenue (#8CDDBE - verde claro)
    if (!isLevelUnlocked(level)) {
      return "#8CDDBE"
    }
    // Si está desbloqueado o completado: color brillante (#00B46F - verde)
    return "#00B46F"
  }

  const getStarColor = (level: number) => {
    // Completado: estrella amarilla #FFBC03 (siempre mostrar si está completado)
  ...
  const handleBackgroundClick = (e: React.MouseEvent<HTMLDivElement>) => {
    // Redirigir a islands/health al hacer click en el fondo
    ...
    >
      ...
      <div className="px-4 pt-8 pb-2 relative">
        <div className="absolute left-10 top-10" onClick={(e) => e.stopPropagation()}>
          <button
            onClick={() => router.push("/islands/health")}
        ...
        <h1 className="text-[#00B46F] text-xl font-bold text-center mt-16">Salud</h1>
      </div>
      ...
      <div className="flex-1 relative px-4 pb-24 pt-2">
        ...
        {Array.from({ length: totalLevels }, (_, i) => {
          ...
          return (
            ...
            >
              ...
              <div
                className={`w-16 h-16 rounded-full flex items-center justify-center text-white font-bold text-xl shadow-lg select-none ${level > currentLevel ? 'level-locked-health' : ''}`}
                style={{
                  backgroundColor: level > currentLevel ? "#8CDDBE" : getLevelColor(level),
                  boxShadow: level === 1 
                    ? "0 4px 12px rgba(0, 180, 111, 0.3)"
                    : "0 4px 12px rgba(140, 221, 190, 0.3)",
                  fontFamily: "var(--font-poppins)",
                  cursor: level > currentLevel ? "not-allowed" : "pointer",
                  WebkitTapHighlightColor: "transparent",
                  userSelect: "none",
                  pointerEvents: level > currentLevel ? "none" : "auto",
                }}
                onClick={(e) => {
        ...
        {/* Popup Card for Level 5 - positioned above level 5 circle */}
        <div
          className="absolute"
          style={{
            top: "30%",
        ...
        >
          ...
          >
            {/* Speech bubble tail pointing down to level 5 circle */}
            <div
              className="absolute"
              style={{
                bottom: "-14px",
            ...
            <div className="flex flex-col items-center gap-1.5" style={{ fontFamily: "var(--font-poppins)" }}>
              ...
              <svg
                ...
                height="50"
                viewBox="0 0 24 24"
                fill={getStarColor(currentLevel)}
              >
              ...
              <div className="flex justify-center mt-2" style={{ marginBottom: "-32px" }}>
                <Link href="/islands/health/level-5/game">
                <button
