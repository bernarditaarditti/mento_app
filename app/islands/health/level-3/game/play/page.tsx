...
export default function HealthLevel3PlayPage() {
  ...
  // Opciones correctas: índice 2 y 3 ("Esto une a la comunidad científica." y "Me recuerda la importancia de cuidar a mis seres queridos.")
  // Opciones incorrectas: índice 0, 1 y 4
  const correctAnswers = [2, 3]
  const incorrectAnswers = [0, 1, 4]
  ...
  const handleSubmit = () => {
    ...
    if (correctAnswers.includes(selectedOption)) {
      ...
      localStorage.setItem("health_level_3_completed", "true")
    }
  }
  ...
  const getErrorMessage = (optionIndex: number | null) => {
    if (optionIndex !== null && incorrectAnswers.includes(optionIndex)) {
      return "Esta interpretación aumenta la emoción negativa y no es constructiva. La idea es encontrar una perspectiva más positiva y útil de la situación"
    }
    return "Esa interpretación no es la más constructiva"
  }
  ...
  return (
    <div className="min-h-screen flex flex-col bg-white">
      ...
      <div className="flex-1 px-4 pb-8 flex flex-col gap-5 max-w-sm mx-auto w-full justify-center pt-0">
        ...
        >
          <div className="flex flex-col gap-3 text-center" style={{ color: "#F36E86", fontFamily: "var(--font-poppins)" }}>
            <p className="text-sm font-medium">
              Titular negativo: Se intensifica la pandemia. Esto te hace sentir frustrado y pesimista, <strong>porque pensás que el mundo nunca va a mejorar</strong>
            </p>
          </div>
        </div>
        ...
        >
          Elegí la opción más constructiva
        </h2>
