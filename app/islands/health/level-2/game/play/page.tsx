...
export default function HealthLevel2PlayPage() {
  ...
  // Opciones correctas (repurposing): índices 3, 4
  // Opciones incorrectas: índices 0, 1, 2, 5
  const correctAnswers = [3, 4]
  const incorrectAnswers = [0, 1, 2, 5]
  ...
  const handleSubmit = () => {
    ...
    // Verificar si todas las opciones seleccionadas son correctas
    const allSelectedAreCorrect = selectedOptions.every(opt => correctAnswers.includes(opt))
    
    if (allSelectedAreCorrect) {
      // Todas las opciones seleccionadas son correctas - éxito
      setShowSuccessPopup(true)
      localStorage.setItem("health_level_2_completed", "true")
    } else {
      // Hay opciones incorrectas seleccionadas - error
  ...
  const handleSuccessPopupClick = async () => {
    ...
    const levelNum = levelMatch ? parseInt(levelMatch[1], 10) : 2
  ...
  return (
    <div className="min-h-screen flex flex-col bg-white">
      ...
      <div className="flex-1 px-4 pb-8 flex flex-col gap-5 max-w-sm mx-auto w-full justify-center pt-0">
        ...
        >
          <div className="flex flex-col gap-3 text-center" style={{ color: "#F36E86", fontFamily: "var(--font-poppins)" }}>
            <p className="text-sm font-medium">
              Aparece una notificación médica: <span className="font-bold">Colesterol alto detectado</span>
            </p>
          </div>
        </div>
        ...
        >
          Elegí todas las reinterpretaciones que correspondan
        </h2>
            ...
            <div 
              ...
              onClick={() => {
                if (typeof window !== "undefined") {
                  sessionStorage.setItem("repurposing_return_path", pathname)
                }
                router.push("/repurposing")
              }}
            >
