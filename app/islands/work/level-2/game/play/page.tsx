...
export default function WorkLevel2PlayPage() {
  ...
  // Opciones correctas: "Me cuesta pero puedo aprender" (índice 2), "Aprendí qué temas reforzar para la próxima" (índice 3), "Al menos tengo una base para avanzar" (índice 4)
  // Opciones incorrectas: "Soy un fracaso" (índice 0), "Esto demuestra que el esfuerzo no sirve" (índice 1), "Siempre me va mal en todo" (índice 5)
  const correctAnswers = [2, 3, 4]
  const incorrectAnswers = [0, 1, 5]
  ...
  const handleSubmit = () => {
    ...
    if (hasCorrect) {
      ...
      // Marcar el nivel 2 como completado en localStorage
      localStorage.setItem("work_level_2_completed", "true")
    } else {
  ...
  const handleSuccessPopupClick = async () => {
    ...
    console.log(`🎮 Guardando progreso nivel 2: isla=${idIsla}, nivel=${levelNum}`)
  ...
  const getOptionStyle = (index: number) => {
    ...
    // Si está seleccionada pero aún no evaluada, mostrar color azul
    if (selectedOptions.includes(index) && !evaluated) {
      return { backgroundColor: "#0096C7" } // Azul cuando está seleccionada
    }
  ...
  return (
    <div className="min-h-screen flex flex-col bg-white">
      ...
      <div className="flex-1 px-4 pb-24 flex flex-col gap-5 max-w-sm mx-auto w-full justify-center pt-10">
        ...
        >
          <div className="flex flex-col gap-3" style={{ color: "#F36E86", fontFamily: "var(--font-poppins)" }}>
            <p className="font-medium text-center" style={{ fontSize: "15px" }}>
              Estudiaste mucho para un examen pero te sacaste un 5
            </p>
          </div>
        </div>
        ...
        >
          Seleccioná las frases que cumplan con la consigna y dejá pasar las que no
        </h2>
