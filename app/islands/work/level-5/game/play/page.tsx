...
export default function WorkLevel5PlayPage() {
  ...
  // Opciones correctas: "Mejoramos aunque no llegamos al objetivo." (índice 0), "Aprendí a coordinar mejor en equipo." (índice 5)
  // Opciones incorrectas: "Fue un desastre total, no sirve para nada." (índice 1), "Seguro el profesor nos tiene bronca." (índice 2), "No aprendimos nada, solo perdimos tiempo." (índice 3), "No voy a trabajar en grupos nunca más." (índice 4)
  const correctAnswers = [0, 5]
  const incorrectAnswers = [1, 2, 3, 4]
  ...
  const handleSubmit = () => {
    ...
    if (hasCorrect) {
      ...
      localStorage.setItem("work_level_5_completed", "true")
    } else {
        ...
        >
          <div className="flex flex-col gap-3" style={{ color: "#F36E86", fontFamily: "var(--font-poppins)" }}>
            <p className="font-medium text-center" style={{ fontSize: "15px" }}>
              El proyecto grupal salió mal y el profesor lo calificó bajo
            </p>
          </div>
        </div>
