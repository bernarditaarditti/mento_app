...
export default function WorkLevel1PlayPage() {
  ...
  useEffect(() => {
    ...
    console.log("🔍 Verificando usuario en useEffect:", { authLoading, user: user ? "existe" : "null" })
    ...
    if (!authLoading && !user) {
      ...
      if (savedUserStr) {
        try {
          ...
          // El usuario existe en localStorage pero no en el contexto
          // Esto podría indicar un problema con el contexto
        } catch (e) {
          console.error("❌ Error al parsear usuario de localStorage:", e)
        }
      } else {
        console.warn("⚠️ No hay usuario en localStorage ni en contexto, redirigiendo a login...")
        router.push("/login")
      }
    }
  }, [authLoading, user, router])
  
  // Opciones correctas: 3 y 4 (índices 2 y 3)
  // Opciones incorrectas: 1 y 2 (índices 0 y 1)
  const correctAnswers = [2, 3]
  const incorrectAnswers = [0, 1]
  ...
  const handleSubmit = () => {
    ...
    if (correctAnswers.includes(selectedOption)) {
      ...
      // El progreso se guarda en la base de datos cuando se cierra el popup
    }
  }

  const handleSuccessPopupClick = async () => {
    // Obtener el usuario del contexto o de localStorage como respaldo
    let currentUser = user
    
    // Si no hay usuario en el contexto pero está cargando, esperar un poco
    if (authLoading) {
    ...
    // Si aún no hay usuario en el contexto, intentar obtenerlo de localStorage y actualizar el contexto
    if (!currentUser || !currentUser.id_usuario) {
      if (typeof window !== "undefined") {
        ...
        if (savedUserStr) {
          try {
            const savedUser = JSON.parse(savedUserStr)
            if (savedUser && savedUser.id_usuario) {
              console.log("⚠️ Usuario encontrado en localStorage pero no en contexto. Actualizando contexto...")
              // Actualizar el contexto con el usuario de localStorage
              login(savedUser)
              currentUser = savedUser
              console.log("✅ Contexto actualizado con usuario de localStorage")
            }
          } catch (e) {
            console.error("❌ Error al parsear usuario de localStorage:", e)
          }
    ...
    if (!currentUser || !currentUser.id_usuario) {
      console.error("❌ No hay usuario disponible en contexto ni en localStorage. Redirigiendo a login...")
      router.push("/login")
      return
    }

    // Continuar con el flujo usando el usuario (del contexto o de localStorage)
    ...
    console.log(`🎮 Guardando progreso: isla=${idIsla}, nivel=${levelNum}`)
    console.log(`👤 Usuario que se usará:`, currentUser)
    ...
    try {
      ...
      if (!resultSave.success) {
        console.error("❌ Error al guardar progreso:", resultSave.message)
      }
    } catch (e: any) {
  ...
  const getOptionStyle = (index: number) => {
    ...
    if (selectedOption === index && !evaluated) {
      return { backgroundColor: "#0096C7" } // Azul oscuro cuando está seleccionada
    }
    
    // Estado inicial: azul claro
    return { backgroundColor: "#71C7E2" }
  }
    ...
    return (
      ...
      >
        ...
        <div className="flex items-center justify-center flex-1 pt-20">
          <Image
            src="/images/cerebro-estudio.png"
            alt="Cerebro estudio"
            width={235}
            height={201}
  ...
  return (
    <div className="min-h-screen flex flex-col bg-white">
      ...
      <div className="flex-1 px-4 pb-8 flex flex-col gap-5 max-w-sm mx-auto w-full justify-center pt-0">
        ...
        >
          ¿Cómo podrías reinterpretar la situación?
        </h2>
        ...
        <div className="flex flex-col gap-3 relative max-w-[90%] mx-auto">
          {options.map((option, index) => (
            <button
              ...
              onMouseEnter={(e) => {
                // No hacer nada si hay popups activos
                if (showErrorPopup || showSuccessPopup) return
                
                if (!evaluated && selectedOption !== index) {
                  e.currentTarget.style.backgroundColor = "#0096C7"
                }
                // Mostrar popup si está evaluado y no hay popup grande activo
                if (evaluated && !showErrorPopup && !showSuccessPopup) {
              ...
              onMouseLeave={(e) => {
                ...
                if (!evaluated && selectedOption !== index) {
                  e.currentTarget.style.backgroundColor = "#71C7E2"
                }
      ...
      {showErrorPopup && (
        ...
        >
          <div className="relative w-full max-w-[70%]" onClick={(e) => e.stopPropagation()}>
            <div
      ...
      {/* Popup para opciones después de evaluar - NO mostrar si hay popup grande activo */}
