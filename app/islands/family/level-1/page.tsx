...
export default function FamilyLevel1Page() {
  ...
  useEffect(() => {
    ...
    for (let i = 1; i <= 5; i++) {
      if (localStorage.getItem(`family_level_${i}_completed`) === "true") {
        completed.add(i)
      }
    }
    if (completed.size > 0) {
  ...
  const getLevelColor = (level: number) => {
    // Si está bloqueado: color tenue (#FFDF87 - amarillo claro)
    if (!isLevelUnlocked(level)) {
      return "#FFDF87"
    }
    // Si está desbloqueado o completado: color brillante (#FFC832 - amarillo oscuro)
    return "#FFC832"
  }
  ...
  const handleBackgroundClick = (e: React.MouseEvent<HTMLDivElement>) => {
    // Redirigir a islands/family al hacer click en el fondo
    router.push("/islands/family")
  }
        ...
        {Array.from({ length: totalLevels }, (_, i) => {
          ...
          return (
            ...
            >
              ...
              <div
                className={`w-16 h-16 rounded-full flex items-center justify-center text-white font-bold text-xl shadow-lg select-none ${!isLevelUnlocked(level) ? 'level-locked' : ''}`}
                style={{
          ...
          >
            ...
            <div className="flex flex-col items-center gap-1.5" style={{ fontFamily: "var(--font-poppins)" }}>
              ...
              <div className="flex justify-center mt-2" style={{ marginBottom: "-32px" }}>
                <Link href="/islands/family/level-1/game">
                <button
