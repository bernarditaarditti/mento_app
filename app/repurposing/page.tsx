...
export default function RepurposingPage() {
  ...
  useEffect(() => {
    ...
    const savedPath = sessionStorage.getItem("repurposing_return_path")
    if (savedPath) {
    ...
    if (typeof window !== "undefined" && document.referrer) {
      try {
        ...
        if (referrerPath.includes("/game/play/")) {
          ...
          sessionStorage.setItem("repurposing_return_path", referrerPath)
        }
      } catch (e) {
        ...
        <div className="relative w-full max-w-[290px] flex flex-col items-center rounded-2xl p-6" style={{ backgroundColor: "#FFC7D1", opacity: 0.82 }}>
          <div className="flex flex-col items-center justify-start" style={{ fontFamily: "var(--font-poppins)" }}>
            ...
            <p className="text-center leading-relaxed px-4" style={{ color: "#F36E86", fontSize: "17px" }}>
              Este nivel usa <strong style={{ color: "#F36E86" }}>"Repurposing"</strong>,
              <br />
              <br />
              una forma de regular tus emociones <strong style={{ color: "#F36E86" }}>cambiando el propósito</strong> que le das a una situación.
              <br />
              <br />
              En lugar de verla solo como algo molesto o difícil, la reinterpretás como una <strong style={{ color: "#F36E86" }}>oportunidad</strong>: para aprender, practicar paciencia, fortalecer un valor, o crecer de alguna manera.
            </p>
          </div>
        </div>
      </div>
