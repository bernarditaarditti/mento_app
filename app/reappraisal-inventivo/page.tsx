...
export default function ReappraisalInventivoPage() {
  ...
  useEffect(() => {
    ...
    const savedPath = sessionStorage.getItem("reappraisal_inventivo_return_path")
    if (savedPath) {
    ...
    if (typeof window !== "undefined" && document.referrer) {
      try {
        ...
        if (referrerPath.includes("/game/play/")) {
          ...
          sessionStorage.setItem("reappraisal_inventivo_return_path", referrerPath)
        }
      } catch (e) {
        ...
        <div className="relative w-full max-w-[290px] flex flex-col items-center rounded-2xl p-6" style={{ backgroundColor: "#FFC7D1", opacity: 0.82 }}>
          <div className="flex flex-col items-center justify-start" style={{ fontFamily: "var(--font-poppins)" }}>
            ...
            <p className="text-center leading-relaxed px-4" style={{ color: "#F36E86", fontSize: "17px" }}>
              Este nivel usa <strong style={{ color: "#F36E86" }}>"Reappraisal inventivo"</strong>,
              <br />
              <br />
              una estrategia en la que generás una interpretación nueva, <strong style={{ color: "#F36E86" }}>creativa</strong> o <strong style={{ color: "#F36E86" }}>inesperada</strong> sobre una situación negativa, para cambiar la emoción que te provoca.
              <br />
              <br />
              No se trata de negar la realidad, sino de buscar activamente alternativas, aunque <strong style={{ color: "#F36E86" }}>no sean las más obvias</strong>.
            </p>
          </div>
        </div>
      </div>
