...
export default function ReinterpretacionPositivaPage() {
  ...
  useEffect(() => {
    ...
    const savedPath = sessionStorage.getItem("reinterpretacion_positiva_return_path")
    if (savedPath) {
    ...
    if (typeof window !== "undefined" && document.referrer) {
      try {
        ...
        if (referrerPath.includes("/game/play/")) {
          ...
          sessionStorage.setItem("reinterpretacion_positiva_return_path", referrerPath)
        }
      } catch (e) {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center bg-white relative">
      <div className="relative w-full max-w-md flex flex-col items-center pt-8 px-4">
        ...
        {/* Pink Info Box */}
        <div className="relative w-full max-w-[320px] flex flex-col items-center rounded-2xl p-6" style={{ backgroundColor: "#FFC7D1", opacity: 0.82 }}>
          <div className="flex flex-col items-center justify-start" style={{ fontFamily: "var(--font-poppins)" }}>
            ...
            <p className="text-center leading-relaxed px-4" style={{ color: "#F36E86", fontSize: "17px" }}>
              Este nivel usa <strong style={{ color: "#F36E86" }}>"Reinterpretación<br />positiva"</strong>,
              <br />
              <br />
              una estrategia que busca encontrar un <strong style={{ color: "#F36E86" }}>aspecto constructivo</strong> o beneficioso <strong style={{ color: "#F36E86" }}>dentro de una situación difícil</strong>. No es negar lo negativo, sino reconocer que puede haber algo bueno mezclado: un aprendizaje, una oportunidad, un fortalecimiento personal.
            </p>
          </div>
        </div>
      </div>
